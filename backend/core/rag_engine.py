...
Pipeline de 3 capas:
  ...
  Capa 3: LLM solo para FORMA (redacción), NUNCA para contenido
"""
...
class RAGEngine:
    ...
    async def query(
        ...
        """
        Procesar una consulta del vendedor a través del pipeline RAG.

        Returns:
            Dict con: reply, fuentes, semaforo, certeza_global, capa_utilizada, productos_mencionados
        """
        ...
        if certainty_level == "Baja" and settings.WEB_SEARCH_ENABLED:
            ...
            if web_results:
                ...
                certainty_level = "Media"  # Web search eleva a Media como máximo

        # ── Paso 4: Redactar respuesta con LLM (solo forma) ─
        ...
        # ── Paso 5: Calcular semáforo ──────────────────────
        semaforo = self._calculate_semaforo(
        ...
        # ── Paso 7: Extraer productos mencionados ──────────
    ...
    async def _search_curated(self, question: str) -> List[Dict]:
        """
        Capa 1: Búsqueda en base curada (ChromaDB).
        Devuelve los sub-chunks más relevantes.
        """
        # TODO: Implementar búsqueda real en ChromaDB
        # query_embedding = self.embedding_engine.encode(question)
        # results = chromadb_client.query(query_embedding, top_k=settings.RAG_TOP_K)
        return []
    ...
    async def _generate_reply(
        ...
        """
        Capa 3: Generar respuesta usando LLM.
        El LLM SOLO redacta la respuesta con los datos proporcionados.
        ...
        system_prompt = (
            ...
            "y herramientas de diagnóstico automotor con 40 años en Argentina. "
            "Tu trabajo es asistir al vendedor con información precisa sobre "
            "compatibilidad de escáneres, multímetros, osciloscopios y otras "
            "herramientas de diagnóstico.\n\n"
            "REGLAS ESTRICTAS:\n"
            "1. Solo usa la información proporcionada en el CONTEXTO. "
            ...
            "4. Si el semáforo es AMARILLO, incluye la advertencia explícita.\n"
            "5. Si el semáforo es ROJO, NO recomiendes el producto para ese uso.\n"
            "6. Cita las fuentes cuando sea posible.\n"
            "7. Responde en español argentino, cercano pero profesional.\n"
        )
        ...
        try:
            ...
            # Si el LLM falla, devolver datos crudos
            if curated_data:
                return self._format_raw_data(curated_data)
            return "Error al generar respuesta. Por favor, consulta las fichas técnicas."
        ...
        if curated_data:
            ...
            for i, item in enumerate(curated_data, 1):
                ...
                parts.append(
                    ...
                    f"Semáforo: {meta.get('semaforo', 'N/A')}"
                )
    ...
    def _calculate_semaforo(
        self,
        curated_results: List[Dict],
        question: str,
    ) -> Dict[str, Any]:
        """
        Calcular semáforo según reglas de negocio.

        REGLAS:
        - IMMO = AMARILLO siempre (no confiable)
        - BSI fuera de cobertura = NO APLICA (no es ROJO)
        - Datos completos y verificados = VERDE
        - Datos parciales o solo web = AMARILLO
        - Incompatibilidad confirmada = ROJO
        """
        if not curated_results:
            return {
                "color": "amarillo",
                "motivo": "Sin datos en base curada",
                "detalle": "Consultar ficha técnica o contacto con fabricante",
            }

        # Verificar si la pregunta es sobre IMMO
        question_lower = question.lower()
        if "immo" in question_lower or "inmovilizador" in question_lower:
            return {
                "color": "amarillo",
                "motivo": "Función IMMO no es confiable en todas las marcas/modelos",
                "detalle": "Solo ofrecer con advertencia explícita al cliente. "
                           "No garantizar cobertura IMMO completa.",
            }

        # Verificar semáforo de los resultados
        semaforos = [r.get("metadata", {}).get("semaforo") for r in curated_results]
        if "rojo" in semaforos:
            return {
                "color": "rojo",
                "motivo": "Incompatibilidad confirmada",
                "detalle": "No ofrecer este producto para este vehículo/sistema.",
            }
        if "amarillo" in semaforos:
            return {
                "color": "amarillo",
                "motivo": "Datos parciales o cobertura limitada",
                "detalle": "Ofrecer con advertencia.",
            }
        if "no_aplica" in semaforos:
            return {
                "color": "no_aplica",
                "motivo": "Fuera de cobertura de gama",
                "detalle": "El equipo no cubre ese sistema. No es incompatibilidad, es fuera de alcance.",
            }

        return {
            "color": "verde",
            "motivo": "Datos verificados y completos",
    ...
    def _extract_products(self, curated_data: List[Dict]) -> List[str]:
    ...
    def _format_raw_data(self, curated_data: List[Dict]) -> str:
        """Formatear datos crudos cuando LLM no está disponible."""
        lines = []
        for item in curated_data:
            lines.append(f"- {item.get('content', 'Sin datos')}")
            meta = item.get("metadata", {})
            lines.append(f"  (Certeza: {meta.get('certezza', 'N/A')} | Semáforo: {meta.get('semaforo', 'N/A')})")
        return "\n".join(lines)
